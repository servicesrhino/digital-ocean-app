...
//   const getParts = (searchTerm, page = 0, pageSize = 20) => {
...
//             ? {
//                 ...row,
//                 id: row.id || `${page}-${index}`, // unique ID per page
...
//               }
//             : { ...row, id: row.id || `${page}-${index}`, routeItemVeh: '' }
//         );

//         setAllData(processed);
//         // setRowCount(500); // If API gives a count, use it here instead of hardcoding
//       })
//       .catch(console.error);
//   };

//   // const getParts = (searchTerm, page, pageSize) => {
//   //   if (searchTerm.length > 0 && searchTerm.length < 4) return;
...
//   //     })
//   //     .then((res) => {
//   //       const checkedValue = res.data.map((row, index) =>
//   //         row.actions[0]?.item?.vehicle
//   //           ? {
//   //               ...row,
//   //               id: row.id || index,
...
//   const handleGetParts = () => {
//     // if (quickFilter.trim().length >= 4) {
//     //   getParts(quickFilter.trim(), page, pageSize); // include pagination state
//     // }
...
//     getParts(trimmed, page, pageSize);
//   };
...
//               getParts(quickFilter, page, pageSize);
//             }}
//             // rowCount={undefined} ← omit this if you don't know total
//             // hide the footer row count summary (optional)
